...
COSMETIC_FORME_SPECIES: dict[str, CosmeticFormes] = {
    ...
    "Vivillon": CosmeticFormes(
        "Meadow", [], None
    ),  # Technically, Fancy and Poke Ball are alt forms and not cosmetic, but eff that
    ...
    for field_init in init_list:
        ...
        match field_name:
            ...
            case "formSpeciesIdTable":
                ...
                # Check if this specific mon needs a form suffix
                if mon["num"] in table:
                    form_name = table[mon["num"]]
                    if form_name != "Base":
                        # Set form suffix for ALL form variants
                ...
                # Handle formeOrder and other form-related fields for the base form
                # (the first entry in the table is considered the base for organizing purposes)
                if list(table.keys())[0] == mon["num"]:
                    ...
                    # Ugly Urshifu hack
                    if mon["name"].split('-')[0] == "Urshifu":  # Handle both Urshifu and Urshifu-Single-Strike
                        mon["formeOrder"] = [
                            mon["name"].split('-')[0]
                            + (
                                f'-{table_vals[i].replace("-Style", "")}'
                                if i > 0
                                else ""
                            )
                    ...
                    # Ugly Xerneas hack
                    elif mon["name"].split('-')[0] == "Xerneas":
                        mon["formeOrder"] = ["Xerneas", "Xerneas-Neutral"]
                        mon["baseForme"] = "Active"
                    # Ugly Vivillon hack
                    elif mon["name"].split('-')[0] == "Vivillon":
                        # expansion stores vivillon icy snow as the default form; showdown expects meadow to be the default
                        mon["formeOrder"] = [
                            f'{mon["name"].split("-")[0]}',
                            f'{mon["name"].split("-")[0]}-Icy-Snow',
                        ]
                        mon["formeOrder"].extend(
                            [
                                f'{mon["name"].split("-")[0]}-{table_vals[i]}'
                                for i in range(len(table.keys()))
                                if table_vals[i]
                                not in (
                                    "Base",
                                    COSMETIC_FORME_SPECIES.get(mon["name"].split('-')[0], None).base if COSMETIC_FORME_SPECIES.get(mon["name"].split('-')[0]) else None,
                                )
                            ]
                        )
                    # Ugly Minior hack
                    elif mon["name"].split('-')[0] == "Minior":
                        # expansion stores minior-meteor-red as the default form; showdown indexes core-red as the default
                        mon["formeOrder"] = [f'{mon["name"].split("-")[0]}', f'{mon["name"].split("-")[0]}-Meteor']
                        mon["formeOrder"].extend(
                            [
                                f'{mon["name"].split("-")[0]}-{table_vals[i]}'
                                for i in range(len(table.keys()))
                    ...
                    # Ugly Zygarde hack
                        ...
                        mon["baseForme"] = "50%"
                    # Ugly Greninja hack
                    elif mon["name"].split('-')[0] == "Greninja":
                        mon["formeOrder"] = ["Greninja", "Greninja-Ash"]
                        mon["baseForme"] = "Base"
                    else:
                        mon["formeOrder"] = [
                            mon["name"].split('-')[0] + (f"-{table_vals[i]}" if i > 0 else "")
                            for i in range(len(table.keys()))
                        ]
                        ...
                        if cosmetics.alts is not None:
                            mon["cosmeticFormes"] = [
                                ...
                                and (
                                    ...
                                    or not re.match(
                                        cosmetics.exclude_pattern, table_vals[i]
                                    )
                                )
                            ]
            ...
            case "evolutions":
                ...
                for i, evo_method in enumerate(field_expr.init.exprs):
                    ...
                    if (
                        method_id == ExpansionEvoMethod.SPECIFIC_MAP.value
                    ):  # TODO:: Leafeon, Glaceon
                        continue
                    if method_id == 0:
                        continue
                    evos.append(
                        [
                            ExpansionEvoMethod(method_id),
                            extract_int(evo_method.exprs[1]),
                            extract_int(evo_method.exprs[2]),
                        ]
                    )
...
def zip_evos(
    all_data: dict[int, tuple[PokemonData, list]],
    items: list[str],
    moves: list[str],
    ...
    for _, (mon, evos) in all_data.items():
        ...
        for i, evo in enumerate(evos):
            ...
            if (
                mon["name"] == "Milcery"
                and parent_mon["name"].startswith("Alcremie")
                and i > 0
            ):
                continue
            ...
            match method:
                ...
                case (
                    ExpansionEvoMethod.SPECIFIC_MON_IN_PARTY
...
def zip_learnsets(
    lvlup_learnset: dict[str, list[int]], teach_learnset: dict[str, list[str]]
) -> dict:
...
def parse_species_data(
    ...
    for species_init in species_data:
        try:
            ...
            if lvlup_learnset or teach_learnset:
                all_learnsets[key]["learnset"] = zip_learnsets(
                    lvlup_learnset, teach_learnset
                )
    ...
    for mon, _ in all_species_data.values():
        ...
        if included_mons:
            mon["tier"] = (
                "obtainable" if mon["name"] in included_mons else "unobtainable"
            )
