...
from porydex.move_descriptions import load_move_descriptions, enrich_moves_with_descriptions
...
def index(moves: dict, species: dict, learnsets: dict, encounters: dict):
    ...
    abilities = json.load(
        open(vanilla_data_dir / "abilities.json", "r", encoding="utf-8")
    )
    items = json.load(open(vanilla_data_dir / "items.json", "r", encoding="utf-8"))
    typechart = json.load(
        open(vanilla_data_dir / "typechart.json", "r", encoding="utf-8")
    )

    _, custom_abilities = load_move_descriptions()
    ...
    # Enrich moves with descriptions
    ...
    index.extend(
        list(map(lambda s: s + " category", ["physical", "special", "status"]))
    )
    index.extend(
        list(
            map(
    ...
    def alias(id: str, name: str, T: str) -> list[str]:
        ...
        if name.endswith("-Mega-X") or name.endswith("-Mega-Y"):
            ...
            index.append(
                f"mega{name_key(name[-1])} {T} {id} {len(name_key(name[:-7]))}"
            )
            return index
        ...
        if name.startswith("Hidden Power "):
            ...
            abbrev = name_key(
                "".join(
                    map(
                        lambda s: s[1] if s[0] != 0 else s[1][0],
    ...
    [
        index.extend(alias(id, ability["name"], "ability"))
    ...
    for T in typechart.keys():
        battle_count_index[f"{T} move"] = sum(
            map(lambda m: int(m["type"].lower() == T), moves.values())
        )
        battle_count_index[f"{T} pokemon"] = sum(
            map(lambda s: int(T.upper() in s["types"]), species.values())
        )
    ...
    with open(porydex.config.output / "search-index.js", "w", encoding="utf-8") as outf:
        ...
        print(
            f"exports.BattleSearchIndex = {json.dumps(battle_search_index, ensure_ascii=False)};",
            file=outf,
            end="\n\n",
        )
        print(
            f"exports.BattleSearchIndexOffset = {json.dumps(battle_offset_index, ensure_ascii=False)};",
            file=outf,
            end="\n\n",
        )
        print(
            f"exports.BattleSearchCountIndex = {json.dumps(battle_count_index, ensure_ascii=False)};",
            file=outf,
            end="\n\n",
        )
    ...
    with open(porydex.config.output / "moves.js", "w+", encoding="utf-8") as outf:
        ...
        for idx, (move_id, m) in enumerate(moves.items(), start=1):
            transformed.append({"id": m["num"], "name": m["name"], "type": typechart})
