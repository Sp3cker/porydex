...
static void fprint_json_string(FILE *f, struct String s)
{
    fputc('"', f);
    for (int i = 0; i < s.string_n; i++)
    {
...
static void fprint_trainers_json(FILE *f, struct Parsed *parsed)
{
    fprintf(f, "[\n");
    ...
    for (int i = 0; i < parsed->trainers_n; i++)
    {
        ...
        // Trainer ID
        fprintf(f, "    \"id\": ");
        fprint_json_string(f, trainer->id);
        fprintf(f, ",\n");

        // Name
        fprintf(f, "    \"name\": ");
        if (!is_empty_string(trainer->name))
            fprint_json_string(f, trainer->name);
        else
            fprintf(f, "null");
        fprintf(f, ",\n");
        ...
        if (!is_empty_string(trainer->class))
            fprint_json_string(f, trainer->class);
        else
            fprintf(f, "null");
        fprintf(f, ",\n");
        ...
        if (!is_empty_string(trainer->pic))
            fprint_json_string(f, trainer->pic);
        else
            fprintf(f, "null");
        fprintf(f, ",\n");
        ...
        // Gender
        fprintf(f, "    \"gender\": ");
        if (trainer->gender == GENDER_MALE)
            fprintf(f, "\"Male\"");
        else if (trainer->gender == GENDER_FEMALE)
            fprintf(f, "\"Female\"");
        else
            fprintf(f, "\"Any\"");
        fprintf(f, ",\n");
        ...
        if (!is_empty_string(trainer->encounter_music))
            fprint_json_string(f, trainer->encounter_music);
        else
            fprintf(f, "null");
        fprintf(f, ",\n");
        ...
        // AI Flags
        fprintf(f, "    \"aiFlags\": [");
        for (int j = 0; j < trainer->ai_flags_n; j++)
        {
            if (j > 0) fprintf(f, ", ");
            fprint_json_string(f, trainer->ai_flags[j]);
        }
        fprintf(f, "],\n");

        // Items
        fprintf(f, "    \"items\": [");
        for (int j = 0; j < trainer->items_n; j++)
        {
            if (j > 0) fprintf(f, ", ");
            fprint_json_string(f, trainer->items[j]);
        }
        fprintf(f, "],\n");
        ...
        if (!is_empty_string(trainer->mugshot))
            fprint_json_string(f, trainer->mugshot);
        else
            fprintf(f, "null");
        fprintf(f, ",\n");
        ...
        if (!is_empty_string(trainer->difficulty))
            fprint_json_string(f, trainer->difficulty);
        else
            fprintf(f, "\"Normal\"");
        fprintf(f, ",\n");
        ...
        for (int j = 0; j < trainer->pokemon_n; j++)
        {
            ...
            // Species
            fprintf(f, "        \"species\": ");
            fprint_json_string(f, pokemon->species);
            fprintf(f, ",\n");

            // Nickname
            fprintf(f, "        \"nickname\": ");
            if (!is_empty_string(pokemon->nickname))
                fprint_json_string(f, pokemon->nickname);
            else
                fprintf(f, "null");
            fprintf(f, ",\n");
            ...
            if (!is_empty_string(pokemon->item))
                fprint_json_string(f, pokemon->item);
            else
                fprintf(f, "null");
            fprintf(f, ",\n");
            ...
            // Ability
            fprintf(f, "        \"ability\": ");
            if (!is_empty_string(pokemon->ability))
                fprint_json_string(f, pokemon->ability);
            else
                fprintf(f, "null");
            fprintf(f, ",\n");
            ...
            // Nature
            fprintf(f, "        \"nature\": ");
            if (!is_empty_string(pokemon->nature))
                fprint_json_string(f, pokemon->nature);
            else
                fprintf(f, "null");
            fprintf(f, ",\n");
            ...
            // Ball
            fprintf(f, "        \"ball\": ");
            if (!is_empty_string(pokemon->ball))
                fprint_json_string(f, pokemon->ball);
            else
                fprintf(f, "null");
            fprintf(f, ",\n");

            // Friendship
            fprintf(f, "        \"friendship\": ");
            if (pokemon->friendship_line)
                fprintf(f, "%d", pokemon->friendship);
            else
                fprintf(f, "null");
            fprintf(f, ",\n");

            // Shiny
            fprintf(f, "        \"shiny\": %s,\n", pokemon->shiny ? "true" : "false");

            // Dynamax Level
            fprintf(f, "        \"dynamaxLevel\": ");
            if (pokemon->dynamax_level_line)
                fprintf(f, "%d", pokemon->dynamax_level);
            else
                fprintf(f, "null");
            fprintf(f, ",\n");

            // Gigantamax Factor
            fprintf(f, "        \"gigantamaxFactor\": %s,\n", pokemon->gigantamax_factor ? "true" : "false");

            // Tera Type
            fprintf(f, "        \"teraType\": ");
            if (!is_empty_string(pokemon->tera_type))
                fprint_json_string(f, pokemon->tera_type);
            else
                fprintf(f, "null");
            fprintf(f, ",\n");

            // EVs - array format: [hp, attack, defense, specialAttack, specialDefense, speed]
            fprintf(f, "        \"evs\": [%d, %d, %d, %d, %d, %d],\n",
                pokemon->evs.hp,
                pokemon->evs.attack,
                pokemon->evs.defense,
            ...
            // IVs - array format: [hp, attack, defense, specialAttack, specialDefense, speed]
            fprintf(f, "        \"ivs\": [%d, %d, %d, %d, %d, %d],\n",
                pokemon->ivs.hp,
                pokemon->ivs.attack,
                pokemon->ivs.defense,
            ...
            {
                if (k > 0) fprintf(f, ", ");
                fprint_json_string(f, pokemon->moves[k]);
            }
            fprintf(f, "],\n");

            // Tags
            fprintf(f, "        \"tags\": [");
            for (int k = 0; k < pokemon->tags_n; k++)
            {
                if (k > 0) fprintf(f, ", ");
                fprint_json_string(f, pokemon->tags[k]);
            }
            fprintf(f, "]\n");
...
int main(int argc, char *argv[])
{
    ...
    int opt;
    while ((opt = getopt(argc, argv, "i:o:j")) != -1)
    {
        switch (opt)
        {
        case 'i':
    ...
    if (json_output)
        fprint_trainers_json(output_file, &parsed);
    else
